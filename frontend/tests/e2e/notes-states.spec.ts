...
for (const scenario of ['empty', 'error', 'slow'] satisfies ListScenario[]) {
  test(`Notas diferencia o estado ${scenario}`, async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('heading', { name: 'Notas fiscais', exact: true })).toBeVisible()

    if (scenario === 'empty') {
      await expect(page.getByRole('heading', { name: 'Nenhuma nota encontrada', exact: true })).toBeVisible()
      await expect(page.getByText('Não foi possível carregar notas', { exact: true })).toHaveCount(0)
    } else if (scenario === 'error') {
      await expect(page.getByText('Não foi possível carregar notas', { exact: true })).toBeVisible()
...
test('Notas navega para próxima e anterior pelo teclado', async ({ page }, testInfo) => {
  ...
  await page.keyboard.press('ArrowDown')
  await expect(page).toHaveURL(new RegExp(`/notes/${SECOND_NOTE_ACCESS_KEY}`))
  await expect(page.getByRole('button', { name: NOTE_ACCESS_KEY, exact: true })).toBeVisible()
  await page.keyboard.press('ArrowUp')
  await expect(page).toHaveURL(new RegExp(`/notes/${NOTE_ACCESS_KEY}`))
})

test('Notas trata chave inexistente ou de outro escritório como não encontrada', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'desktop-1440', 'Contrato 404 independe da largura.')
  await installApiFixtures(page, 'ADMIN')
  await page.goto('/notes/CHAVE-INEXISTENTE')
