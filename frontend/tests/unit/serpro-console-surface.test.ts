...
describe('console global SERPRO (superfície)', () => {
  ...
  it('API platform aponta para rotas canônicas sanitizadas', async () => {
    ...
    expect(calls).toContain('/api/v1/platform/serpro/production-onboarding')
  ...
  it('páginas admin/serpro existem e não reexibem segredos', () => {
    ...
    expect(configPage).toContain('serpro-production-onboarding')
    expect(configPage).toContain('productionOnboarding.submit')
