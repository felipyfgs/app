...
describe('plataforma /admin e seletor global (6.2)', () => {
  ...
  it('admin entra por Escritórios e as superfícies internas seguem Lista/Settings', () => {
    ...
    expect(identity).toContain('useGlobalPlatformSelector')
    expect(identity).toContain('me.value?.has_real_membership !== true')
    expect(identity).toContain('void switchTo(targetOfficeId)')
...
describe('procuração e franquia na UI', () => {
  ...
  it('SITFIS expõe franquia, procuração e modal de refresh', () => {
    ...
    const table = readFileSync(resolve(APP, 'utils/sitfis-table.ts'), 'utf8')
    const commercial = readFileSync(
      resolve(APP, 'components/monitoring/CommercialMetaCell.vue'),
      'utf8'
    )
    expect(table).toContain('MonitoringCommercialMetaCell')
    expect(table).toContain('ClientsClientProcuracaoBadge')
    expect(table).toContain('w-28 min-w-24')
    expect(table).toContain('${findings} · ${pending} pend.')
    expect(table).not.toContain('finding(s)')
    expect(table).not.toContain('w-56')
    expect(table).toContain('tableIconGroup')
    expect(sitfis).toContain('min-w-[720px]')
    expect(sitfis).toContain(':initial-hidden-columns="[\'procuracao\', \'franchise\', \'age\', \'observed\']"')
    expect(sitfis).toContain('buildSitfisColumns')
