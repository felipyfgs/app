...
/** Checklist CT-e derivado do arquétipo settings do dashboard oficial. */
import type { CteHealth, CteOnboarding, CtePendingItem } from '~/types/api'
...
const health = ref<CteHealth | null>(null)
...
const checklist = computed(() => [
  ...
  {
    label: 'Stream do escritório inicializado',
    done: Number(health.value?.summary.office_streams || 0) > 0,
    detail: `${health.value?.summary.office_streams || 0} stream(s) do escritório.`
  }
])
...
async function load() {
  ...
  loading.value = true
  try {
    const [onboardingResponse, healthResponse, pendingResponse] = await Promise.all([
      api.cte.onboarding(),
      api.cte.health(),
      api.cte.pending()
    ])
    ...
    health.value = null
    pending.value = []
    loadError.value = apiErrorMessage(caught, 'Não foi possível carregar o onboarding CT-e.')
  } finally {
  ...
  try {
    await api.quarantine.resolve(item.id, {
      ...
      resolution_notes: resolution === 'RESOLVED'
        ? 'Revisado no painel CT-e (sem automação de portal).'
        : 'Descartado no painel CT-e.'
    })
    toast.add({
...
<template>
  <div class="space-y-6 lg:space-y-8" data-testid="cte-onboarding-page">
    <UPageCard
      variant="naked"
      orientation="horizontal"
      title="Onboarding CT-e"
      description="Prepare o autXML do escritório para novos CT-e e acompanhe os canais sem expor PFX, senha ou XML fiscal."
    >
      <UButton
    ...
    <UAlert
      ...
      title="Ações sensíveis exigem 2FA"
      description="Cadastro de A1, tokens de integração e outras mutações de cofre ficam em Administração, com segundo fator confirmado. Esta tela só mostra metadados públicos e pendências."
    />
    ...
    >
      ...
      <span class="sr-only">Carregando checklist CT-e…</span>
    </div>

    <UEmpty
      v-else-if="!loading && !loadError && !onboarding"
      icon="i-lucide-truck"
      title="Onboarding CT-e indisponível"
    ...
    <template v-else-if="onboarding">
      ...
      <UPageCard title="Checklist operacional" variant="subtle">
        <ul class="divide-y divide-default" aria-label="Checklist operacional CT-e">
          <li
      ...
      <div data-testid="cte-pending-panel">
        <UPageCard
          ...
          description="Itens abertos do modelo 57 / schema CTe. Resolução manual apenas — sem automação de portais públicos."
          variant="subtle"
        >
          <div
      ...
      <UAlert
        ...
        :actions="[{ label: 'Importar XML/ZIP', to: '/docs', color: 'neutral', variant: 'outline' }]"
      />
    </template>
  </div>
</template>
