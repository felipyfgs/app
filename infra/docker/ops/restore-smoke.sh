...
BACKUP_PACKAGE_KEY="$package_key" \
    ./infra/docker/ops/backup.sh "$backup_root" >/dev/null
...
STACK_PROJECT="$project" \
BACKUP_PACKAGE_KEY="$package_key" \
    ./infra/docker/ops/restore.sh --verify-only "$backup_dir" >/dev/null
...
if BACKUP_PACKAGE_KEY="$(openssl rand -base64 32)" \
    ...
    STACK_ENV="$env_file" \
    STACK_PROJECT="$project" \
    ./infra/docker/ops/restore.sh --verify-only "$backup_dir" >/dev/null 2>&1; then
...
STACK_WEB=web \
BACKUP_PACKAGE_KEY="$package_key" \
    ./infra/docker/ops/restore.sh --force "$backup_dir" >/dev/null
