...
#   ./infra/docker/ops/verify.sh
#   ./infra/docker/ops/verify.sh --full
#   ./infra/docker/ops/verify.sh --preflight-strict
set -euo pipefail
...
if [[ -x "$ROOT/infra/docker/ops/secret-scan.sh" ]]; then
  if ! "$ROOT/infra/docker/ops/secret-scan.sh"; then
    if [[ "$FULL" -eq 1 ]]; then
