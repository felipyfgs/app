...
phase_source() {
  ...
  for script in infra/docker/ops/deploy.sh infra/docker/ops/backup.sh infra/docker/ops/restore.sh \
                infra/docker/ops/prod-readiness.sh infra/docker/ops/secret-scan.sh; do
    if [[ -f "$script" ]]; then
...
phase_predeploy() {
  ...
  if [[ -f "$BACKUP_ENV" ]]; then
    ...
    if [[ -f "infra/docker/ops/backup.env.example" ]]; then
...
usage() {
  cat >&2 <<'EOF'
Uso: PHASE=source|predeploy|postdeploy|all infra/docker/ops/prod-readiness.sh
