...
`Situação · Últ. Declaração · [valores/domínio] · Cliente · Ações · Envio · Hist. comunicação · Consulta`.

Carteiras **sem** Últ. Declaração (PGMEI, SITFIS, FGTS, MIT) SHALL usar:
`Situação · Cliente · [domínio] · Ações · Envio · Hist. comunicação · Consulta`.
...
- **THEN** a ordem é Situação · Últ. Declaração · RBT12 · Cliente · Ações · Envio · Hist. comunicação · Consulta

### Requirement: Colunas Envio e Hist. comunicação distintas

A coluna **Envio** SHALL conter apenas Send (prévia + confirmação de envio manual) e Switch que persiste `automatic_requested` para envio automático após consulta agendada. A coluna **Hist. comunicação** SHALL conter apenas o ícone de rastreio local. A coluna **Ações** SHALL conter apenas o menu ⋮ (sem ícones de preview/info de comunicação na grade). O envio efetivo via provider MUST permanecer fail-closed (desligado por default). Preferências de canal MUST ser editáveis via item do ⋮ (não somente leitura).
...
- **AND** não há ícones de mensagem/info de comunicação na grade
...
SITFIS, FGTS e a cápsula MIT SHALL expor as colunas Envio e Hist. comunicação e endpoints de preferência/prévia/rastreio/send no contrato do módulo (wrappers sobre o core de comunicação), com enrichment de `communication` no portfolio quando disponível (fail-closed se ausente).

#### Scenario: MIT tem colunas de envio e rastreio

- **WHEN** o operador está na cápsula MIT
- **THEN** a grade inclui Envio e Hist. comunicação na spine
