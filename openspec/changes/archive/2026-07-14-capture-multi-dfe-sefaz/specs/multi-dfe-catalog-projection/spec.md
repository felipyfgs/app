...
O sistema SHALL projetar documentos capturados nos kinds `NFE`, `NFCE` (quando aplicável), `CTE` e `MDFE` com campos comuns de catálogo (chave, número, partes, valor, datas, status, papel, kind, source) além das projeções NFS-e existentes.

#### Scenario: Listagem multi-tipo
- **WHEN** o cliente chama `GET /api/v1/documents` sem filtro de kind
- **THEN** a resposta pode incluir itens de múltiplos kinds capturados, cada um com `kind` e `kind_label` corretos
...
O sistema SHALL vincular eventos (NF-e, CT-e, MDF-e) à chave de acesso correspondente e SHALL atualizar a situação derivada da projeção sem alterar o XML original do documento principal.
