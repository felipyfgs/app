...
No frontend Nuxt, a implementação usará o arquétipo do template já fixado. Documentos (`/docs/catalog`, contexto `kind=CTE`) exibirá o checklist para incluir o CNPJ copiável do escritório em `autXML`, cobertura, import em lote, pendências/quarentena e metadados seguros; Sincronizações terá cards separados para CT-e do cliente e CT-e `autXML` (cursor/`maxNSU`/quiet/`656`); Configurações e a navegação MUST NOT ter destino CT-e próprio. ADMIN com 2FA recente controla A1/flags e resolução sensível na Administração; OPERATOR importa e reprocessa itens elegíveis; VIEWER é somente leitura.
