...
- [ ] 15.5 Selecionar emitente piloto que inclua previamente o CNPJ do escritório em `autXML` sem alterar documento já autorizado.
- [ ] 15.6 Capturar o primeiro CT-e pelo stream do escritório e confirmar roteamento, qualidade, referências `999...` e resultado real da assinatura.
- [ ] 15.7 Importar original do mesmo piloto quando disponível e confirmar reconciliação com a cópia `autXML` sem perda de proveniência.
