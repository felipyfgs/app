...
- **THEN** a página envia `sort` e `sort_direction` ao endpoint de guias, recarrega a página 1 e atualiza a URL com esses parâmetros
...
Listas N1 com ordenação no servidor (incluindo a visão Por cliente em Documentos e a lista de clientes) SHALL sincronizar `sort` e `sort_direction` na query string. A whitelist de sort da lista de clientes MUST NÃO incluir `cnpj` enquanto não houver coluna CNPJ ordenável.
