...
- [ ] 4.6 Implementar estados e alertas de vencimento em 30, 7 e 1 dia e bloqueio após expiração
- [ ] 4.7 Testar criptografia/adulteração, senha incorreta, raiz divergente, vencimento, rotação e ausência de segredos em respostas/logs
...
- [ ] 5.4 Versionar fixtures sanitizadas dos estados oficiais, lotes GZip/Base64, alertas e rejeições
...
- [ ] 6.4 Implementar parser versionado de NFS-e/eventos, papéis fiscais e situação derivada sem alterar o documento original
...
- [ ] 6.9 Testar idempotência, rollback parcial, duplicatas, fim da distribuição, 429/5xx, payload corrompido e ausência de starvation

## 7. Catálogo e APIs fiscais

- [ ] 7.1 Implementar projeções consultáveis para chave, partes, papel, competência, emissão, valores e situação
- [ ] 7.2 Preservar documentos bem-formados com XSD desconhecido em estado de revisão sem bloquear o cursor
- [ ] 7.3 Implementar `GET /api/v1/notes` com cursor pagination e filtros combináveis
- [ ] 7.4 Implementar detalhe e download de XML original por chave, aplicando escritório/perfil e auditoria
- [x] 7.5 Implementar endpoints de histórico de sincronização e disparo manual
- [ ] 7.6 Cobrir notas emitidas, tomadas, intermediadas, eventos, competência versus emissão e acesso cruzado com testes

## 8. Exportações XML

- [x] 8.1 Criar migration/modelo de exportações com estados, filtros, expiração e ownership por escritório/usuário
- [ ] 8.2 Implementar criação assíncrona de ZIP com opção de eventos e paths `CNPJ/AAAA-MM/papel/chave.xml`
...
- [ ] 8.5 Testar filtros, deduplicação, paths seguros, inclusão de eventos, expiração e isolamento entre escritórios
...
- [ ] 9.2 Criar fluxo de login/2FA e navegação Dashboard, Clientes, Notas, Exportações, Sincronizações e Administração condicionada ao perfil
- [ ] 9.3 Implementar dashboard com cards de métricas, falhas, filas, validade de certificados e horários de atualização
- [ ] 9.4 Implementar fluxo guiado cliente → estabelecimento → A1 → teste → primeira sincronização
- [ ] 9.5 Implementar tabelas de clientes e notas com filtros server-side, colunas responsivas e paginação por cursor
- [ ] 9.6 Implementar detalhe de nota, filtros separados de competência/emissão e download individual
- [ ] 9.7 Implementar solicitação, acompanhamento e download de exportações
- [ ] 9.8 Implementar telas/slideover de histórico, alertas e estados vazios/erro sem revelar informações sensíveis
...
- [ ] 10.1 Implementar auditoria de autenticação, cadastros, certificados, sincronizações manuais, downloads e exportações
- [ ] 10.2 Adicionar logs estruturados com correlação e redaction, métricas de Horizon/ADN/disco e healthchecks restritos
...
- [ ] 10.4 Executar análise estática, testes backend/frontend e varredura automatizada de segredos nas imagens e artefatos
