...
use App\Contracts\SefazMdfeDistDfeClient;
...
use App\Services\Sefaz\MdfeDistDfePageProcessor;
...
/**
 * Sincroniza DistDFe MDF-e para um channel_sync_cursor (flag sefaz.mdfe_enabled).
 */
class SyncSefazMdfeDistDfeJob implements ShouldQueue
{
    use Queueable;
    ...
    public function __construct(
        ...
        $this->onQueue((string) config('sefaz.queues.mdfe', 'sync-sefaz-mdfe'));
    }

    public function handle(
        SefazMdfeDistDfeClient $client,
        MdfeDistDfePageProcessor $processor,
        CredentialService $credentials,
    ): void {
        if (! config('sefaz.mdfe_enabled')) {
            return;
        }
        ...
        if ($cursor === null || $cursor->channel !== CaptureChannel::MdfeDistDfe) {
            return;
        }

        $lock = Cache::lock(
            'sefaz:mdfe:est:'.$cursor->establishment_id,
        ...
        try {
            ...
            if (! $establishment || ! $clientModel) {
                throw new AdnPermanentException('Estabelecimento/cliente ausente para MDF-e DistDFe.');
            }
            ...
            $credential = ClientCredential::query()
                ...
                ->first();
            if (! $credential) {
                throw new AdnPermanentException('Credencial A1 ativa ausente para MDF-e DistDFe.');
            }

            $material = $credentials->loadPfxMaterial($credential);
            if ($material === null) {
                throw new AdnPermanentException('Não foi possível materializar A1 para MDF-e DistDFe.');
            }
            ...
            $this->failCursor($cursor, 'Falha MDF-e DistDFe: '.mb_substr($e->getMessage(), 0, 200), permanent: false);
            throw $e;
        } finally {
            $lock->release();
        }
