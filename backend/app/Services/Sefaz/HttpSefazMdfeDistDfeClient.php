...
/**
 * Cliente MDF-e DistDFe — SOAP 1.2 + mTLS PFX BLOB.
 */
final class HttpSefazMdfeDistDfeClient implements SefazMdfeDistDfeClient
{
    ...
    private function call(array $certificate, string $distDfeIntXml): DistDfePageDto
    {
        ...
        $ns = (string) config('sefaz.mdfe.namespace', 'http://www.portalfiscal.inf.br/mdfe/wsdl/MDFeDistribuicaoDFe');
        $envelope = <<<XML
<?xml version="1.0" encoding="UTF-8"?>
<soap12:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap12="http://www.w3.org/2003/05/soap-envelope">
  <soap12:Body>
    <mdfeDistDFeInteresse xmlns="{$ns}">
      <mdfeDadosMsg>{$distDfeIntXml}</mdfeDadosMsg>
...
XML;
        ...
        $response = $this->transport->post($url, $certificate, $envelope, [
            'SOAPAction: "'.(string) config('sefaz.mdfe.soap_action', $ns.'/mdfeDistDFeInteresse').'"',
        ]);

        $status = $response['status'];
        if ($status >= 500) {
            throw new AdnRetryableException('SEFAZ MDF-e DistDFe temporariamente indisponível.');
        }
        if ($status >= 400) {
            throw new AdnPermanentException('SEFAZ MDF-e DistDFe rejeitou a requisição HTTP.');
        }
    ...
    {
        $versao = (string) config('sefaz.mdfe.layout_version', '1.00');
        ...
        return '<distDFeInt xmlns="http://www.portalfiscal.inf.br/mdfe" versao="'.$versao.'">'
            .'<tpAmb>'.$tpAmb.'</tpAmb>'
    ...
    private function endpointUrl(): string
    {
        ...
        return (string) config('sefaz.mdfe.'.$key);
    }
    ...
    private function normalizeCnpj(string $cnpj): string
    {
        ...
        if (strlen($c) !== 14) {
            throw new AdnPermanentException('CNPJ de consulta MDF-e DistDFe inválido.');
        }
