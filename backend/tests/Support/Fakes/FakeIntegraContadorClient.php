...
final class FakeIntegraContadorClient implements IntegraContadorClient
{
    ...
    private function defaultResponse(IntegraRequest $request): IntegraResponse
    {
        ...
        if ($opKey === 'sicalc.consultaapoioreceitas') {
            $code = (string) ($request->businessData['codigoReceita'] ?? '1082');
            $dados = [
                'receita' => [
                    'codigoReceita' => $code,
                    'descricaoReceita' => 'IRRF - Rendimentos do trabalho assalariado',
                    'extensoes' => [[
                        'obrigatorios' => ['codigoReceita' => true, 'dataPA' => true, 'valorImposto' => true],
                        'opcionais' => ['referencia' => true, 'observacao' => true],
                        'informacoes' => [
                            'calculado' => true,
                            'codigoBarras' => true,
                            'codigoReceitaExtensao' => '0',
                            'descricaoReceitaExtensao' => 'Receita sem extensão adicional',
                            'pf' => true,
                            'pj' => true,
                        ],
                    ]],
            ...
            return new IntegraResponse(
                success: true,
                httpStatus: 200,
                body: ['status' => '200', 'dados' => json_encode($dados, JSON_THROW_ON_ERROR)],
        ...
        if ($opKey === 'pagtoweb.contaconsdocarrpg') {
            return new IntegraResponse(
                success: true,
                httpStatus: 200,
                body: ['status' => '200', 'dados' => '1'],
                headers: ['x-simulated' => '1'],
                simulated: true,
                correlationId: $request->correlationId,
                latencyMs: 1,
        ...
        if ($opKey === 'pagtoweb.pagamentos') {
            $dados = ['pagamentos' => [[
                'numeroDocumento' => '12345678901234567', 'tipo' => ['codigo' => '1', 'descricaoAbreviada' => 'DARF'],
                'dataArrecadacao' => '2026-01-15T00:00:00-03:00', 'dataVencimento' => '2026-01-20T00:00:00-03:00',
                'receitaPrincipal' => ['codigo' => '1082', 'descricao' => 'Receita simulada'], 'valorTotal' => 123.45,
            ]]];

            return new IntegraResponse(success: true, httpStatus: 200, body: ['status' => '200', 'dados' => json_encode($dados, JSON_THROW_ON_ERROR)], headers: ['x-simulated' => '1'], simulated: true, correlationId: $request->correlationId, latencyMs: 1, dados: $dados, operationKey: $opKey);
        }
        ...
        $period = (string) ($request->businessData['competencia']
            ...
            ?? $request->businessData['anoCalendario']
            ?? $request->businessData['ano']
    ...
    private function sitfisResponse(IntegraRequest $request): IntegraResponse
    {
        ...
        if ($request->operationKey === 'sitfis.solicitar_protocolo') {
            $protocol = 'SIM-'.substr(hash('sha256', $request->resolvedRequestTag()), 0, 20);
        ...
        return new IntegraResponse(
            ...
            $base + [
                'status' => 200,
                'dados' => ['layoutVersion' => '1.0', 'situacao' => 'REGULAR', 'pendencias' => []],
            ],
            simulated: true,
            correlationId: $request->correlationId,
            dados: ['layoutVersion' => '1.0', 'situacao' => 'REGULAR', 'pendencias' => []],
    ...
    private function simplesMeiBody(
        ...
        if ($service === 'DEFIS') {
            ...
            if ($operation === 'CONSULTAR_DECLARACAO_RECIBO') {
                return [
                    'status' => 200,
                    'mensagens' => [],
                    'dados' => json_encode([
            ...
            if ($operation === 'CONSULTAR_ULTIMA_DECLARACAO_RECIBO') {
                return [
                    'status' => 200,
                    'mensagens' => [],
                    'dados' => json_encode([
                        'ano' => (int) $year,
                        // A fixture exercita a sanitização; este identificador nunca sai do adapter.
            ...
            if ($operation === 'CONSULTAR') {
                return [
                    ...
                    'mensagens' => [],
                    'dados' => json_encode([[
                        'anoCalendario' => (int) $year,
                        'idDefis' => '000000002025001',
                        'tipo' => '1',
                        'dataHora' => $year.'0101000000',
                    ]], JSON_THROW_ON_ERROR),
                ];
            }
            $status = match ($force) {
        ...
        if ($service === 'REGIME_APURACAO') {
            if ($operation === 'CONSULTAR') {
                // O payload chega normalizado no parâmetro $period; não há
                // IntegraRequest neste escopo. Manter o ano solicitado evita
                // que a fixture mascare a validação fail-closed do serviço 103.
                $year = preg_match('/^\d{4}$/', $period) === 1 ? (int) $period : (int) now()->year;
            ...
            if ($operation === 'CONSULTAR_ANOS_CALENDARIOS') {
                return [
                    'status' => 200,
                    'dados' => json_encode([
            ...
            if ($operation === 'CONSULTAR_RESOLUCAO') {
                $text = 'Resolução do Regime de Caixa (fixture fake) — texto fixo sem dados sensíveis.';
                $yearDigits = preg_replace('/\D/', '', $period) ?? '';
                $year = strlen($yearDigits) >= 4 ? (int) substr($yearDigits, 0, 4) : (int) now()->year;
        ...
        if ($service === 'CCMEI') {
            ...
            if ($operation === 'CONSULTAR_SITUACAO_CADASTRAL') {
                return [
                    'dto_version' => '1',
                    'data' => [[
                        'cnpj' => '00000000000000',
                        'situacao' => $status === 'ATIVO' ? 'ATIVA' : $status,
                        'enquadradoMei' => $status === 'ATIVO',
                    ]],
                ];
            }
    ...
    private function domainTripleFromOperationKey(string $operationKey): array
    {
        return match (true) {
            ...
            $operationKey === 'defis.consultimadecrec' => ['INTEGRA_SN', 'DEFIS', 'CONSULTAR_ULTIMA_DECLARACAO_RECIBO'],
            $operationKey === 'defis.consdecrec' => ['INTEGRA_SN', 'DEFIS', 'CONSULTAR_DECLARACAO_RECIBO'],
            str_starts_with($operationKey, 'defis.') => ['INTEGRA_SN', 'DEFIS', 'CONSULTAR'],
            $operationKey === 'regimeapuracao.consultaranoscalendarios' => ['INTEGRA_SN', 'REGIME_APURACAO', 'CONSULTAR_ANOS_CALENDARIOS'],
            $operationKey === 'regimeapuracao.consultarresolucao' => ['INTEGRA_SN', 'REGIME_APURACAO', 'CONSULTAR_RESOLUCAO'],
            ...
            $operationKey === 'ccmei.ccmeisitcadastral' => ['INTEGRA_MEI', 'CCMEI', 'CONSULTAR_SITUACAO_CADASTRAL'],
