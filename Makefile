...
help:
	...
	@echo "  make seed-pilot         Seed piloto (dados reais de .local/dados/)"
...
# Seed normal: exemplos sintéticos (DatabaseSeeder). Não usa .local/dados/.
seed seed-dev:
	docker compose exec php php artisan db:seed --force

# Seed piloto: Felipe/Gustavo + CNPJs de .local/dados/ (PilotSeeder).
...
prod-restore-smoke:
	./infra/docker/ops/restore-smoke.sh
...
prod-release-manifest:
	RELEASE_SHA="$(RELEASE_SHA)" RELEASE_TAG="$(RELEASE_TAG)" PROD_ENV="$(PROD_ENV)" \
		./infra/docker/ops/release-manifest.sh
